...
},
{
  ...
  "body": " Diffie-Hellman key exchange   Alice and Bob wish to share a key over an insecure communication channel. The Diffie-Hellman key exchange accomplishes this as follows.   Public  Alice and Bob agree publicly on a prime and a primitive root .    Private  Alice chooses a random integer exponent , and Bob chooses a random integer exponent . These exponents are kept private.    Exchange  Alice computes and sends this value to Bob; Bob computes and sends this value to Alice.    Shared key  Alice uses her private exponent to compute ; and Bob uses his private exponent to compute . Since , Alice and Bob are now in possession of a shared key .      "
},
{
...
},
{
  ...
  "body": " Discrete log and Diffie-Hellman problems     It is clear that a solution to the discrete log problem could be used to solve the Diffie-Hellman problem. The converse is unknown: that is, it is not known whether a solution to the Diffie-Hellman problem would give rise to a solution to the discrete log problem.    The disrete log problem for general prime is considered to be difficult to solve in general. In particular there is no known algorithm for computing discrete logs modulo whose runtime is polynomial in .    The discrete log problem can be solved efficiently if has small prime factors, thanks to the Pohlig-Hellman algorithm (an application of the CRT). Since is always even (for large primes), a Diffie-Hellman key exchange protocol should us a prime such that , where is a large prime, or equivalently, . Such primes are called safe primes in the cryptographic literature.     "
},
{
  ...
  "body": " Diffie-Hellman implementation  Let's implement the Diffie-Hellman key exchange protocol, taking some care to choose a large safe prime. We first create a list of primes of the form of a largish size, where is prime.   Next let's perform a key exchange with one of these primes. We will do our arithmetic within , as opposed to using congruences.   Now Alice and Bob will choose their random private exponents, exchange their computed powers, and compute the shared key.   Could Eve have determined by solving the discrete log problem in this case? Let's see if Sage can do it. We try to use Sage's built-in discrete log function generalised_log .   "
},
{
  "id": "d_elgamal",
  "level": "2",
  ...
  "body": " Elgamal encryption   Bob wishes to send messages to Alice over an insecure channel. The Elgamal cryptosystem accomplishes this as follows.   Public  Alice publishes a large prime , a primitive root modulo .    Private  Alice chooses a random integer exponent , and keeps this exponent private.    Public key  Alice computes and publishes this value as her public key.    Encryption  To encrypt a message , Bob chooses a random integer , computes and , and sends the ciphertext to Alice.    Decryption  To decrypt the message, Alice uses her private key to compute .      "
},
{
