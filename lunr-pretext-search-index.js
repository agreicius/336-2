...
},
{
  ...
  "body": " Fermat's last theorem and the method of descent     Contrast the wealth of solutions to with the paucity of solution for .    Introduce Fermat's method of descent, and understand it as a version of the well-ordering principle.    Prove the case of Fermat's last theorem.     We saw in the last section that are many interesting positive solutions to the Pythagorean equation . Interestingly, the situation changes dramatically if we increase the exponent 2 in that equation to anything larger.   Fermat's last theorem (FLT)   Let be an integer satisfying . The equation has no solutions in nonzero integers.    In the 17th century Fermat famously scrawled in his copy of Diophantus's Arithmetica that he had a marvelous proof of this fact that unfortunately did not fit in the margin. Although Fermat did provide a proof of the case of FLT using his method of descent (see below), there is no evidence that he had a valid proof for all cases. Moreover, amazingly it was not until 1994 that a complete proof was provided and accepted by the mathematical community as valid. This was the work of Andrew Wiles, which built off the work of many others ( , Gerhard Frey, Ken Ribet, and Richard Taylor), and which made use of very deep results that drew connections with elliptic curves and modular curves.   FLT:   The equation has no solutions in nonzero integers. As a result Fermat's last theorem holds for .    First observe that if is a nontrivial solution to the Fermat equation , then is a nontrivial solution to . Thus the given result implies FLT for .  We argue by contradiction. If is a solution in nonzero integers, then is a solution in positive integers. As such the set is nonempty. By the well-ordering principle, has a least element , which means there is a nontrivial solution in positive integers to the equation whose third component is minimal. We will derive a contradiction by showing that there is a solution in positive integers with .  First observe that if the prime is a common divisor of and , then , in which case and is a solution to with smaller third component. As this is a contradiction, we may assume that . It follows that , and hence that the Pythagorean triple is primitive. Without loss of generality, we may assume that is even and is odd, in which case we have for positive integers and satisfying , , and .  Since and , the triple is a primitive Pythagorean triple. Furthermore, since is odd, we see that is even, and we have for relatively prime positive integers and . Substituting into the equation above, we see that . Thus , and we have . Next, since and with , we see that , , and are pairwise relatively prime. Since their product is a square, it follows from (and induction) that , , and are all squares. Let , , and . From the equation we conclude that , and thus that is a solution to in positive integers. Since and since , we conclude that . Thus the solution has a smaller third component than , contradicting the minimality of .    "
},
{
  "id": "s_flt_descent-2",
  "level": "2",
...
},
{
  ...
  "body": " FLT:   The equation has no solutions in nonzero integers. As a result Fermat's last theorem holds for .    First observe that if is a nontrivial solution to the Fermat equation , then is a nontrivial solution to . Thus the given result implies FLT for .  We argue by contradiction. If is a solution in nonzero integers, then is a solution in positive integers. As such the set is nonempty. By the well-ordering principle, has a least element , which means there is a nontrivial solution in positive integers to the equation whose third component is minimal. We will derive a contradiction by showing that there is a solution in positive integers with .  First observe that if the prime is a common divisor of and , then , in which case and is a solution to with smaller third component. As this is a contradiction, we may assume that . It follows that , and hence that the Pythagorean triple is primitive. Without loss of generality, we may assume that is even and is odd, in which case we have for positive integers and satisfying , , and .  Since and , the triple is a primitive Pythagorean triple. Furthermore, since is odd, we see that is even, and we have for relatively prime positive integers and . Substituting into the equation above, we see that . Thus , and we have . Next, since and with , we see that , , and are pairwise relatively prime. Since their product is a square, it follows from (and induction) that , , and are all squares. Let , , and . From the equation we conclude that , and thus that is a solution to in positive integers. Since and since , we conclude that . Thus the solution has a smaller third component than , contradicting the minimality of .   "
},
{
...
{
  "id": "s_complex_numbers-9",
  "level": "2",
  "url": "s_complex_numbers.html#s_complex_numbers-9",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "complex plane "
...
},
{
  ...
  "body": " Factorization in Gaussian integers   Division and Euclidean algorithms   Division algorithm in   Let and be Gaussian integers with . There are Gaussian integers satisfying the following conditions.    .     .   Moreover, and can be produced via the following procedure.   Find satisfying : , set .    Write , where and .    We have , where satisfies .       That the procedure above produces a and in satisfying is clear. It remains only to show that . We have      Division algorithm in   For the given and , find satisfying and .    ,      ,        In both cases we follow the procedure described in .   Set . We pick to be a closest Gaussian integer to this element of . In this case we can pick , in which case . Thus our instance of the division algorithm in this case is . Note that we have , as stipulated.    We have . In this case, since lies in the center of the Gaussian square with vertices , , , and , any one of those vertices can serve as our !  Clearly, is the easiest to deal with, resulting in and a division algorithm statement of . Let see what happens for another choice of . Taking , we have and our division algorithm statement is .        Greatest common divisor and least common multiple   Let be a subring of . Given elements a greatest common divisor (or GCD ) of and is an element satisfying the following conditions:    and ;    if satisfies and , then .     A least common multiple (or LCM ) of and is an element satisfying the following conditions:    and ;    if satisfies and , then .   More generally, a greatest common divisor of elements of is an element such that for all , and if satisfies for all , then . Lastly, we say that elements are relatively prime if they have as a GCD.     GCDs and LCDs  Although the definition of GCDs and LCMs is a straightforward generalization of the corresponding notion from the integers, you should proceed with caution when making use of these concepts. In particular, be aware of the following facts:   GCDs and LCMs need not exist in an arbitrary subring of ;    if GCDs and LCMs do exist, they are only unique modulo multiplication by a unit.   The uniqueness issue is already in evidence in the context of the integers. To nonzero integers will always have two GCDs, namely and , where , as you will recall, is defined to be the greatest positive integer dividing and .    Euclidean algorithm in   Given elements , one of which is nonzero, we can produce a GCD following the Euclidean algorithm below.  If , then is a GCD of and . Otherwise, we can produce a sequence of division algorithm instances , and it follows that is a GCD of and . Moreover, beginning with the penultimate equation , and working our way upward, solving successively for in terms of and , we can find elements satisfying .    First note that if for some , then is a GCD of and if and only if it is a GCD of and , using the usual divisibility arguments.  It follows by induction that if we have a sequence of division algorithm instances as in (2), then is a GCD of and if and only if it is a GCD of It suffices, then, to show that there is such a sequence. The basic idea is that at each step , if , then we can apply the division algorithm to and . Since at each step where the remainder is nonzero, the successive remainder has strictly smaller -value, the process must terminate: , at some point we must have .     GCD in   Find a GCD of and in , and write for some .    Noting that and , we start the Euclidean algorithm by dividing by . . We conclude that is a GCD of and . Furthermore, we have .     Bezout's theorem for   For any two elements , one of which is nonzero, there is a GCD . Furthermore, we can write for elements .     Relatively prime factors   Assume are relatively prime elements of .   If , then .    If and , then .       Since and are relatively prime, we can write some . We will make use of this expression in both parts below.   Assume that for some . By definition this means for some . It follows that , and thus that .    Assume that and for some . By definition, this means and for some . From it now follows that , and we see that .         Primes and irreducibles in   Primes and irreducibles   Let be a subring of , and let be a nonzero non-unit of : , .    is irreducible if implies  .     is prime if implies or .        Disentangling primes and irreducibles  The two separate definitions above might come as a surprise to you, since in the context of the two are equivalent! For a general subring , however, this is not the case. Although it is the case that all prime elements are irreducible, there are rings with irreducible elements that are not prime. Thankfully for us, in as in , the two notions are equivalent.    Prime equivalent to irreducible in   Let be a nonzero non-unit. The following statements are equivalent.    is prime.     is irreducible.       The easier implication is (1) (2); and indeed this implication holds for any subring . Assume is prime. If , then in particular . Since is prime, we have or . If , then we have for some , and hence . Since is nonzero, it has a multiplicative inverse in , allowing us to cancel and conclude . Since , we conclude that . The case where is exactly similar.  The implication (2) (1) is not true for a general subring . As you will see, the key property of we use is the existence of GCDs and Bezout's theorem. Assume is irreducible, and that for . We wish to show that or . To this end, assume that . We claim that and are relatively prime in this case. Indeed, since is irreducible, its only divisors are units and associates of itself. It follows that a common divisor of and would be a unit or an associate of . But since , no associate of divides . We conclude that the only common divisors of and are units, and hence that the two are are relatively prime. From we conclude that , as desired.     Prime\/irreducible  From here on out, we are justified in using the terms prime and irreducible interchangeably when working in . For the sheer sake of brevity, as well as its familiar ring, we will prefer the term prime . However, when emphasizing the fact the two notions in are equivalent in this setting, we will sometimes write prime\/irreducible .   We are nearly prepared to prove a prime factorization theorem for that is a straightforward generalization of the corresponding result for . The existence of prime factorizations will proceed by induction on the Gaussian norm of an element, and for the base case, we will need the following useful result.   Primes, norms, and conjugates   Let be a nonzero non-unit.    is prime if and only if is prime if and only if is prime for any unit .    If for a prime integer, then is prime in .    If is prime in , then , where is a prime integer.          Proof by delegation. (In other words, this is left to you!) The first equivalence follows easily from the fact that . The second follows from the fact that units can be cancelled multiplicatively in equations involving Gaussian integers.    Assume where is a prime integer. We will show is irreducible, hence prime. Given , we have . Thus is irreducible\/prime, as claimed.    Assume is prime\/irreducible. Write , where is an integer and (since is not unit). By prime factorization, we can write for primes . Since in , it divides for some . Call . Since , we have for some . Taking conjugates, we have . But then . Since is prime the only factors of are . Since , we conclude that , as claimed.        Gaussian primes   For each Gaussian integer verify directly whether it is prime or not, and compare your answer with .                          Note that , which implies (indirectly) that these elements are both prime. To verify this directly, it is easiest to show that in this case is irreducible, and this is done using a norm argument identical to the one used in the proof of . Note that , and thus the primality of either element implies the primality of the other. Interestingly, we also have , so these two primes are in fact associates of each other. More on that later. Lastly, the nontrivial factorization shows that , though prime in , is not prime in .  We claim is prime in . If , then . Since the divisors of are , we see that either , or . The first option is impossible, as there are no integers satisfying . The latter implies that is a unit or is a unit, as desired. Note in this case that is prime in , but its norm is not prime. It is, however, equal to the square of a prime, as predicted by .  We first compute . Since is not or for any prime integer , we must have not prime\/irreducible. To verify this directly, note that if is a nontrivial factorization, then . This tells us to look for factors of with norm and , which leads us to the nontrivial factorization .    We are ready at last for our prime factorization theorem for .   Prime factorization in   Let be a nonzero non-unit element.   Prime factorization  There are prime\/irreducible elements of satisfying .    Uniqueness of factorization  If we have , with and prime\/irreducible for all and , then , and after a reordering, for all we have for some unit .  In particular, the number of irreducible factors in an irreducible factorization of is unique, and the irreducible factors are unique up to multiplication by a unit ( , up to associates).          The proof is by strong induction on $N(\\alpha)$, the base case being . (We cannot have , since by assumption is a nonzero non-unit.) If $N(\\alpha)=2$, then is itself prime by , and we are done.  For the strong induction step, we assume the result is true for all Gaussian integers with norm in . Assume . If is prime\/irreducible, we are done. Otherwise, we can write for some non-units . Since , and since , we conclude that . By induction we have for primes . But then is a prime factorization of , as desired.    The uniqueness claim is proved by induction on the minimal length of a prime factorization.  Base step: . In this case we have itself prime\/irreducible. Suppose is another prime factorization. If , then we would be able to write , where and are both non-units, contradicting the irreducibility of . Thus , and we have . This proves the uniqueness claim for the case when is itself prime.    Induction step: we assume that for any with a prime factorization of minimal length , the factorization is unique in the manner described. Suppose has a prime factorization of minimal length of the form and suppose is another prime factorization. Since is minimal, we must have . Since , we must have for some . After a re-ordering, we may assume . We then have for some unit . This allows us to cancel from both sides of the equation, yielding . Since has a prime factorization of minimal length (at most) , induction applies.It follows that , and after a reordering, for some unit for all . But then we have and for all , as desired.       As with factorization in , from a prime factorization of an element , we can produce a prime power factorization by grouping together like primes, which in this case means primes that are associate. For example, from the prime factorization , observing that , we have , expressing as a product of a unit and powers of two non-associate primes . The corollary below gives the general result. The proof is very similar to the analogous result for integers and is left to the reader.   Prime power factorization in   Let be a nonzero non-unit element of .   Prime power factorization  There are pairwise non-associate prime\/irreducible elements ,positive integers , and a unit such that .    Uniqueness of factorization  If is another such factorization into powers of pairwise nonassociate primes , then , and after a reordering, is associate to and .        "
},
{
  "id": "th_div_alg_gauss",
  "level": "2",
...
},
{
  ...
  "body": " Relatively prime factors   Assume are relatively prime elements of .   If , then .    If and , then .       Since and are relatively prime, we can write some . We will make use of this expression in both parts below.   Assume that for some . By definition this means for some . It follows that , and thus that .    Assume that and for some . By definition, this means and for some . From it now follows that , and we see that .      "
},
{
...
},
{
  ...
  "body": " Prime equivalent to irreducible in   Let be a nonzero non-unit. The following statements are equivalent.    is prime.     is irreducible.       The easier implication is (1) (2); and indeed this implication holds for any subring . Assume is prime. If , then in particular . Since is prime, we have or . If , then we have for some , and hence . Since is nonzero, it has a multiplicative inverse in , allowing us to cancel and conclude . Since , we conclude that . The case where is exactly similar.  The implication (2) (1) is not true for a general subring . As you will see, the key property of we use is the existence of GCDs and Bezout's theorem. Assume is irreducible, and that for . We wish to show that or . To this end, assume that . We claim that and are relatively prime in this case. Indeed, since is irreducible, its only divisors are units and associates of itself. It follows that a common divisor of and would be a unit or an associate of . But since , no associate of divides . We conclude that the only common divisors of and are units, and hence that the two are are relatively prime. From we conclude that , as desired.   "
},
{
  "id": "ss_primes_irreducibles-5",
  "level": "2",
  "url": "s_gauss_factorization.html#ss_primes_irreducibles-5",
  "type": "Remark",
  "number": "1.13.12",
  "title": "Prime\/irreducible.",
  "body": " Prime\/irreducible  From here on out, we are justified in using the terms prime and irreducible interchangeably when working in . For the sheer sake of brevity, as well as its familiar ring, we will prefer the term prime . However, when emphasizing the fact the two notions in are equivalent in this setting, we will sometimes write prime\/irreducible .  "
},
{
  "id": "prop_primes_norm",
  "level": "2",
  "url": "s_gauss_factorization.html#prop_primes_norm",
  "type": "Proposition",
  "number": "1.13.13",
  "title": "Primes, norms, and conjugates.",
  "body": " Primes, norms, and conjugates   Let be a nonzero non-unit.    is prime if and only if is prime if and only if is prime for any unit .    If for a prime integer, then is prime in .    If is prime in , then , where is a prime integer.          Proof by delegation. (In other words, this is left to you!) The first equivalence follows easily from the fact that . The second follows from the fact that units can be cancelled multiplicatively in equations involving Gaussian integers.    Assume where is a prime integer. We will show is irreducible, hence prime. Given , we have . Thus is irreducible\/prime, as claimed.    Assume is prime\/irreducible. Write , where is an integer and (since is not unit). By prime factorization, we can write for primes . Since in , it divides for some . Call . Since , we have for some . Taking conjugates, we have . But then . Since is prime the only factors of are . Since , we conclude that , as claimed.      "
},
{
  "id": "eg_gauss_primes",
  "level": "2",
  "url": "s_gauss_factorization.html#eg_gauss_primes",
  "type": "Example",
  "number": "1.13.14",
  "title": "Gaussian primes.",
  "body": " Gaussian primes   For each Gaussian integer verify directly whether it is prime or not, and compare your answer with .                          Note that , which implies (indirectly) that these elements are both prime. To verify this directly, it is easiest to show that in this case is irreducible, and this is done using a norm argument identical to the one used in the proof of . Note that , and thus the primality of either element implies the primality of the other. Interestingly, we also have , so these two primes are in fact associates of each other. More on that later. Lastly, the nontrivial factorization shows that , though prime in , is not prime in .  We claim is prime in . If , then . Since the divisors of are , we see that either , or . The first option is impossible, as there are no integers satisfying . The latter implies that is a unit or is a unit, as desired. Note in this case that is prime in , but its norm is not prime. It is, however, equal to the square of a prime, as predicted by .  We first compute . Since is not or for any prime integer , we must have not prime\/irreducible. To verify this directly, note that if is a nontrivial factorization, then . This tells us to look for factors of with norm and , which leads us to the nontrivial factorization .   "
},
{
  "id": "th_unique_factorization_gauss",
  "level": "2",
  "url": "s_gauss_factorization.html#th_unique_factorization_gauss",
  "type": "Theorem",
  "number": "1.13.15",
  "title": "Prime factorization in <span class=\"process-math\">\\(\\Z[i]\\)<\/span>.",
  "body": " Prime factorization in   Let be a nonzero non-unit element.   Prime factorization  There are prime\/irreducible elements of satisfying .    Uniqueness of factorization  If we have , with and prime\/irreducible for all and , then , and after a reordering, for all we have for some unit .  In particular, the number of irreducible factors in an irreducible factorization of is unique, and the irreducible factors are unique up to multiplication by a unit ( , up to associates).          The proof is by strong induction on $N(\\alpha)$, the base case being . (We cannot have , since by assumption is a nonzero non-unit.) If $N(\\alpha)=2$, then is itself prime by , and we are done.  For the strong induction step, we assume the result is true for all Gaussian integers with norm in . Assume . If is prime\/irreducible, we are done. Otherwise, we can write for some non-units . Since , and since , we conclude that . By induction we have for primes . But then is a prime factorization of , as desired.    The uniqueness claim is proved by induction on the minimal length of a prime factorization.  Base step: . In this case we have itself prime\/irreducible. Suppose is another prime factorization. If , then we would be able to write , where and are both non-units, contradicting the irreducibility of . Thus , and we have . This proves the uniqueness claim for the case when is itself prime.    Induction step: we assume that for any with a prime factorization of minimal length , the factorization is unique in the manner described. Suppose has a prime factorization of minimal length of the form and suppose is another prime factorization. Since is minimal, we must have . Since , we must have for some . After a re-ordering, we may assume . We then have for some unit . This allows us to cancel from both sides of the equation, yielding . Since has a prime factorization of minimal length (at most) , induction applies.It follows that , and after a reordering, for some unit for all . But then we have and for all , as desired.      "
},
{
  "id": "cor_prime_power_fact_gauss",
  "level": "2",
  "url": "s_gauss_factorization.html#cor_prime_power_fact_gauss",
  "type": "Corollary",
  "number": "1.13.16",
  "title": "Prime power factorization in <span class=\"process-math\">\\(\\Z[i]\\)<\/span>.",
  "body": " Prime power factorization in   Let be a nonzero non-unit element of .   Prime power factorization  There are pairwise non-associate prime\/irreducible elements ,positive integers , and a unit such that .    Uniqueness of factorization  If is another such factorization into powers of pairwise nonassociate primes , then , and after a reordering, is associate to and .      "
},
{
  "id": "s_gauss_primes",
  "level": "1",
...
},
{
  "id": "eg_gauss_factor",
  "level": "2",
  "url": "s_gauss_primes.html#eg_gauss_factor",
  "type": "Example",
  "number": "1.14.6",
  "title": "Prime factorization in <span class=\"process-math\">\\(\\Z[i]\\)<\/span>.",
  "body": " Prime factorization in   Give a prime factorization of in .       "
},
{
  "id": "th_integer_sum_squares",
  "level": "2",
  "url": "s_gauss_primes.html#th_integer_sum_squares",
  "type": "Theorem",
  "number": "1.14.7",
...
},
{
  "id": "eg_sum_squares",
  "level": "2",
  "url": "s_gauss_primes.html#eg_sum_squares",
  "type": "Example",
  "number": "1.14.8",
  "title": "Sum of squares.",
  "body": " Sum of squares   Find all pairs satisfying .       "
},
{
  "id": "s_ring_integers",
  "level": "1",
  "url": "s_ring_integers.html",
  "type": "Section",
  "number": "1.15",
  "title": "Quadratic rings of integers",
  "body": " Quadratic rings of integers   Quadratic extensions of   Quadratic extension of   Let be a squarefree integer. We define to be the following subset of : . We call a quadratic field extension of , or a quadratic subfield of .  The quadratic field is real if ( , if ) and imaginary otherwise ( , if ).     Quadratic field extensions  You will likely notice the similarity in notation between quadratic fields and the Gaussian integers . Notice further some important differences: the in the first notation is an indication that the set consists of -linear combinations , as opposed to -linear combinations , . Moreover, the use of parentheses in as opposed to brackets (as in ) is a notational indication that is a field and not just a ring. (See .)  The quadratic fields can be thought of as generalizations of . The analog of in these settings is given by the ring of integers  defined below.   You might be wondering about the importance of this squarefree condition. This will arise at various points in our discussion. Our first example is the simple characterization of equality in given below.   Equality in   Let be a squarefree integer, and let .   For all nonzero , is not a square in .    We have for all and in .          If is negative, then clearly is negative, and hence not a square in . Assume . If for some , then after expressing and as quotients of integers and clearing denominators, we see that we have for some nonzero integers and . Let be any prime divisor of . Since it appears with exponent in the prime factorization of ( is squarefree), it appears with odd exponent in the prime factorization of . But this contradicts the fact that .    The reverse implication is obvious. For the forward implication, we have . If , we would have . But this is impossible: if , then we would have , contradicting (1). Thus we must have , or equivalently . It then follows tautologically that .        Quadratic norm and conjugate   Let be squarefree, and let . For any , we define its conjugate  and norm  as follows: . We call the resulting function the norm function of over .     It is important not to conflate the general conjugate and norm operations of a quadratic field with complex conjugation and the Gaussian norm function.  Take quadratic field , for example. Given any , we have . There operations are clearly different than complex conjugation and the Gaussian norm. For instance we have , whereas the complex conjugate of is itself (since it is real)! Furthermore, note that can assume negative values ( ), whereas the Gaussian norm is a nonnegative function.  That said, in the case where is imaginary ( , ), then the quadratic conjugate and norm operations are in fact identical to complex conjugation and modulus squared.    Quadratic conjugates and norms   Let be a squarefree integer, and let . As usual, given , we define .   For all , we have .    Given , we have .          These are simple algebraic arguments left to the reader.    Observe first that . Now given , we have . For the second part, given , we have .        Quadratic extensions of are fields   Let be a squarefree integer, and let .    is a subring of .    Every nonzero element of has a multiplicative inverse: , is a field . More explicitly, given nonzero , we have .          We need only show that contains and , and is closed under addition, additive inverses ( ), and multiplication. These are simple algebraic arguments left to the reader.    This is an immediate consequence of the fact that implies and the following algebra: .         Ring of integers   Monic polynomial   Let be a ring. A polynomial is monic if its leading coefficient is 1: , if , with for all .     Algebraic integers of   A complex number is an algebraic integer if there is a monic polynomial such that .     Algebraic versus algebraic integer  Recall that an element is algebraic if there is a polynomial such that . The condition for being an algebraic integer is stricter on two counts: we must be able to find a polynomial not just with rational coefficients, but integer ones; furthermore, the polynomial must be monic.  Note in this regard that any algebraic element will satisfy a polynomial with integer coefficients, just by clearing denominators in the polynomial satisfying . For example, the element satisfies the monic rational polynomial and the integer polynomial . However, we will be able to show that does not satisfy a monic integer polynomial.   We will primarily be interested in the algebraic integers lying within one of our quadratic fields . Before we get there, we assemble some general results about algebraic integers.   Algebraic integers form a ring   Let be the set of all algebraic integers in : , .    . In other words, all integers are algebraic integers, and furthermore, the set of rational numbers that are algebraic integers is precisely .     is closed under addition and multiplication: , we have .   As a result, is a subring of .       If , then for the monic integer polynomial . Thus .  Conversely, suppose . Let be a monic polynomial satisfying . Writing with and clearing denominators in the equation , we see that . Since all constants above are integers, we see that , a contradiction since .    As intuitive as this property might sound, we resort to a little linear algebra trickery to prove it. Namely, we first prove the following sufficient condition for an element to be an algebraic integer: if there is a set of complex numbers, not all zero, such that for all we have for some integer constants , then is an algebraic integer.  Before proving this claim, let's see why it yields the desired result. Suppose that and are algebraic integers satisfying and , where and are monic integer polynomials. Since , it follows that the set satisfies the condition above for and . Indeed, we have . But then it follows easily that and also satisfy the claims condition with respect to , showing that these elements are also algebraic integers.       With these generalities out of the way, we can now define the ring of integers of a quadratic field .   Quadratic ring of integers   Let be squarefree, and let . The ring of integers of , denoted is the set of all elements of that are algebraic integers: , . We call a quadratic ring of integers .     Quadratic ring of integers   Let be a squarefree integer, and let .    is a subring of .    Given , we have if and only if and .    We have .          This follows easily from the fact the set of algebraic integers is closed under addition and multiplication, as is .    Note that is a root of the monic polynomial . If and , then and thus .  Conversely, assume , and suppose for the polynomial . Since for all , we see that . Thus is also a root of , showing tha that . It follows that and are elements of . Lastly, since , we see that . From (1) of , we conclude that .    Using the condition from (2), it is easy to see that elements of the rings on the right side of the equality are elements of . It remains to show that the reverse inclusion.  Assume . From (2) it follows that . We have . Since , we have for some integer .  Assume first that is even, so that . It follows that . Using the fact that is square free, we can show that in this case. Indeed, writing with , we have . If had a prime divisor , , and since , we'd have . Thus when is an integer, , and we have .  Now assume with odd. Since , we have . The same argument above implies that , and thus for some integer . But then we have , in which case . But this can happen if and only if . This in turn forces and to both be odd, in which case . Thus in this case we must have , and we have , where since is even.       As with , the multiplicative units of play an important role in our analysis of these rings. We will say much more about these units in the next section. For now we content ourselves with a definition and a first elementary proposition.   Units of   Let be squarefree, and let . An element is a unit (or invertible ) if there is an element such that . The group of units of is the set of all units: , .     Units of   Let be squarefree, and let . An element if and only if . In other words .    We prove both implications separately.  If , then there is an element such that . Taking norms, we conclude that . Since and are both integers by , and since the only divisors of are , we conclude that .  Conversely, if , then we see that and thus is an inverse of in . Since if and only if , we see that , showing that .    When analyzing we made heavy use of the fact that the Gaussian norm function satisfied a division algorithm property. Interestingly, the same cannot be said of a general norm function for the ring of integers . Accordingly, we give a name to those special rings for which this is true.   Norm-Euclidean   Let be squarefree, and let . The norm  satisfies the division algorithm if given with , there exist satisfying the following conditions.    .     .   We also say that is norm-Euclidean in this case.    The importance of a quadratic ring of integer being norm-Euclidean is that we can perform a version of the Euclidean algorithm ( ) using to compute GCDs of elements , and moreover, express a GCD in the form as in Bezout's theorem. As we saw in the discussion of , that fact then leads to a cascade of useful properties we used to classify the prime elements of . As a result, these same properties hold for a norm-Euclidean .   Norm-Euclidean rings of integers   Let be squarefree, and let . If is norm-Euclidean then it enjoys the following properties.   Let and assume or . There is a GCD of and , and we have for some .    An element is prime if and only if it is irreducible.    If is a nonzero non-unit, then it has a factorization where is irreducible for all , and this factorization is unique in the sense of .       The proofs of these statements can be adapted immediately from the proofs of the corresponding results for , all of which build on the fact that the norm satisfies the division algorithm.    Without making things official here, a ring that satisfies the factorization property described in (4) of is called a unique factorization domain , or UFD for short. Thus, the theorem tells us that if a quadratic ring of integers is norm-equivalent, then it is a UFD. Interestingly, although not all of these rings are UFDs, they all do admit irreducible factorizations. Note that we are careful to say irreducible factorizations in this context, as we cannot assume that prime and irreducible are equivalent.   Irreducible factorizations in quadratic rings of integers   Let be a squarefree, let , and let be a nonzero non-unit of . There are irreducible elements , such that .    We will prove by by strong induction on that every element can be factored into irreducibles.  First observe that , and thus that the set is nonempty. Let be the least element of . This is our base case for the induction argument. Assume . If , then , and thus . Since is the minimal absolute norm of elements of , it follows that . But then or is a unit, by . This proves is irreducible.  Now assume the statement is true for all with , and assume . If is irreducible, then we are done. Otherwise, we can write , where and are non-units. It follows that , whence the induction hypothesis applies. We conclude that and have irreducible factorizations. Since , the product of these two irreducible factorizations is an irreducible factorization of .     is not a UFD   Let and consider . (Note that .)   Compute .    Show that , , and are pairwise non-associate irreducible elements of .    Show that is not a UFD.    Show that none of the irreducible elements , , , are prime. Thus prime is not equivalent to irreducible in .          An element is a unit if and only if . It follows easily that the only units of are and .    Since , it follows easily that the listed elements are pairwise non-associate.  We prove is irreducible; the same argument can be used for the remaining elements. If for , then taking norms, we see that . It is easy to see that there are no elements of with norm equal to (or ). Thus we must have , showing that or is a unit, and hence is irreducible.    The factorizations and give us two irreducible factorizations of 6 consisting of non-associate irreducibles. Thus irreducible factorization is not unique in , showing that it is not a UFD.    The two factorizations above also show us that , , , and are not prime. For example, we have , but and .        "
},
{
  "id": "d_",
  "level": "2",
  "url": "s_ring_integers.html#d_",
  "type": "Definition",
  "number": "1.15.1",
  "title": "Quadratic extension of <span class=\"process-math\">\\(\\Q\\)<\/span>.",
  "body": " Quadratic extension of   Let be a squarefree integer. We define to be the following subset of : . We call a quadratic field extension of , or a quadratic subfield of .  The quadratic field is real if ( , if ) and imaginary otherwise ( , if ).   "
},
{
  "id": "ss_quadratic_ext-3",
  "level": "2",
  "url": "s_ring_integers.html#ss_quadratic_ext-3",
  "type": "Remark",
  "number": "1.15.2",
  "title": "Quadratic field extensions.",
  "body": " Quadratic field extensions  You will likely notice the similarity in notation between quadratic fields and the Gaussian integers . Notice further some important differences: the in the first notation is an indication that the set consists of -linear combinations , as opposed to -linear combinations , . Moreover, the use of parentheses in as opposed to brackets (as in ) is a notational indication that is a field and not just a ring. (See .)  The quadratic fields can be thought of as generalizations of . The analog of in these settings is given by the ring of integers  defined below.  "
},
{
  "id": "prop_quad_ext_equality",
  "level": "2",
  "url": "s_ring_integers.html#prop_quad_ext_equality",
  "type": "Proposition",
  "number": "1.15.3",
  "title": "Equality in <span class=\"process-math\">\\(\\Q(\\sqrt{d})\\)<\/span>.",
  "body": " Equality in   Let be a squarefree integer, and let .   For all nonzero , is not a square in .    We have for all and in .          If is negative, then clearly is negative, and hence not a square in . Assume . If for some , then after expressing and as quotients of integers and clearing denominators, we see that we have for some nonzero integers and . Let be any prime divisor of . Since it appears with exponent in the prime factorization of ( is squarefree), it appears with odd exponent in the prime factorization of . But this contradicts the fact that .    The reverse implication is obvious. For the forward implication, we have . If , we would have . But this is impossible: if , then we would have , contradicting (1). Thus we must have , or equivalently . It then follows tautologically that .      "
},
{
  "id": "d_quad_conjugate_norm",
  "level": "2",
  "url": "s_ring_integers.html#d_quad_conjugate_norm",
  "type": "Definition",
  "number": "1.15.4",
  "title": "Quadratic norm and conjugate.",
  "body": " Quadratic norm and conjugate   Let be squarefree, and let . For any , we define its conjugate  and norm  as follows: . We call the resulting function the norm function of over .   "
},
{
  "id": "ss_quadratic_ext-7",
  "level": "2",
  "url": "s_ring_integers.html#ss_quadratic_ext-7",
  "type": "Warning",
  "number": "1.15.5",
  "title": "",
  "body": " It is important not to conflate the general conjugate and norm operations of a quadratic field with complex conjugation and the Gaussian norm function.  Take quadratic field , for example. Given any , we have . There operations are clearly different than complex conjugation and the Gaussian norm. For instance we have , whereas the complex conjugate of is itself (since it is real)! Furthermore, note that can assume negative values ( ), whereas the Gaussian norm is a nonnegative function.  That said, in the case where is imaginary ( , ), then the quadratic conjugate and norm operations are in fact identical to complex conjugation and modulus squared.  "
},
{
  "id": "prop_quad_norm",
  "level": "2",
  "url": "s_ring_integers.html#prop_quad_norm",
  "type": "Proposition",
  "number": "1.15.6",
  "title": "Quadratic conjugates and norms.",
  "body": " Quadratic conjugates and norms   Let be a squarefree integer, and let . As usual, given , we define .   For all , we have .    Given , we have .          These are simple algebraic arguments left to the reader.    Observe first that . Now given , we have . For the second part, given , we have .      "
},
{
  "id": "th_quad_ext_field",
  "level": "2",
  "url": "s_ring_integers.html#th_quad_ext_field",
  "type": "Theorem",
  "number": "1.15.7",
  "title": "Quadratic extensions of <span class=\"process-math\">\\(\\Q\\)<\/span> are fields.",
  "body": " Quadratic extensions of are fields   Let be a squarefree integer, and let .    is a subring of .    Every nonzero element of has a multiplicative inverse: , is a field . More explicitly, given nonzero , we have .          We need only show that contains and , and is closed under addition, additive inverses ( ), and multiplication. These are simple algebraic arguments left to the reader.    This is an immediate consequence of the fact that implies and the following algebra: .      "
},
{
  "id": "d_monic",
  "level": "2",
  "url": "s_ring_integers.html#d_monic",
  "type": "Definition",
  "number": "1.15.8",
  "title": "Monic polynomial.",
  "body": " Monic polynomial   Let be a ring. A polynomial is monic if its leading coefficient is 1: , if , with for all .   "
},
{
  "id": "d_alg_integers",
  "level": "2",
  "url": "s_ring_integers.html#d_alg_integers",
  "type": "Definition",
  "number": "1.15.9",
  "title": "Algebraic integers of <span class=\"process-math\">\\(\\C\\)<\/span>.",
  "body": " Algebraic integers of   A complex number is an algebraic integer if there is a monic polynomial such that .   "
},
{
  "id": "ss_ring_of_integers-4",
  "level": "2",
  "url": "s_ring_integers.html#ss_ring_of_integers-4",
  "type": "Remark",
  "number": "1.15.10",
  "title": "Algebraic versus algebraic integer.",
  "body": " Algebraic versus algebraic integer  Recall that an element is algebraic if there is a polynomial such that . The condition for being an algebraic integer is stricter on two counts: we must be able to find a polynomial not just with rational coefficients, but integer ones; furthermore, the polynomial must be monic.  Note in this regard that any algebraic element will satisfy a polynomial with integer coefficients, just by clearing denominators in the polynomial satisfying . For example, the element satisfies the monic rational polynomial and the integer polynomial . However, we will be able to show that does not satisfy a monic integer polynomial.  "
},
{
  "id": "th_alg_int_ring",
  "level": "2",
  "url": "s_ring_integers.html#th_alg_int_ring",
  "type": "Theorem",
  "number": "1.15.11",
  "title": "Algebraic integers form a ring.",
  "body": " Algebraic integers form a ring   Let be the set of all algebraic integers in : , .    . In other words, all integers are algebraic integers, and furthermore, the set of rational numbers that are algebraic integers is precisely .     is closed under addition and multiplication: , we have .   As a result, is a subring of .       If , then for the monic integer polynomial . Thus .  Conversely, suppose . Let be a monic polynomial satisfying . Writing with and clearing denominators in the equation , we see that . Since all constants above are integers, we see that , a contradiction since .    As intuitive as this property might sound, we resort to a little linear algebra trickery to prove it. Namely, we first prove the following sufficient condition for an element to be an algebraic integer: if there is a set of complex numbers, not all zero, such that for all we have for some integer constants , then is an algebraic integer.  Before proving this claim, let's see why it yields the desired result. Suppose that and are algebraic integers satisfying and , where and are monic integer polynomials. Since , it follows that the set satisfies the condition above for and . Indeed, we have . But then it follows easily that and also satisfy the claims condition with respect to , showing that these elements are also algebraic integers.      "
},
{
  "id": "d_quad_ring_integers",
  "level": "2",
  "url": "s_ring_integers.html#d_quad_ring_integers",
  "type": "Definition",
  "number": "1.15.12",
  "title": "Quadratic ring of integers.",
  "body": " Quadratic ring of integers   Let be squarefree, and let . The ring of integers of , denoted is the set of all elements of that are algebraic integers: , . We call a quadratic ring of integers .   "
},
{
  "id": "th_quad_ring_integers",
  "level": "2",
  "url": "s_ring_integers.html#th_quad_ring_integers",
  "type": "Theorem",
  "number": "1.15.13",
  "title": "Quadratic ring of integers.",
  "body": " Quadratic ring of integers   Let be a squarefree integer, and let .    is a subring of .    Given , we have if and only if and .    We have .          This follows easily from the fact the set of algebraic integers is closed under addition and multiplication, as is .    Note that is a root of the monic polynomial . If and , then and thus .  Conversely, assume , and suppose for the polynomial . Since for all , we see that . Thus is also a root of , showing tha that . It follows that and are elements of . Lastly, since , we see that . From (1) of , we conclude that .    Using the condition from (2), it is easy to see that elements of the rings on the right side of the equality are elements of . It remains to show that the reverse inclusion.  Assume . From (2) it follows that . We have . Since , we have for some integer .  Assume first that is even, so that . It follows that . Using the fact that is square free, we can show that in this case. Indeed, writing with , we have . If had a prime divisor , , and since , we'd have . Thus when is an integer, , and we have .  Now assume with odd. Since , we have . The same argument above implies that , and thus for some integer . But then we have , in which case . But this can happen if and only if . This in turn forces and to both be odd, in which case . Thus in this case we must have , and we have , where since is even.      "
},
{
  "id": "d_quad_ring_units",
  "level": "2",
  "url": "s_ring_integers.html#d_quad_ring_units",
  "type": "Definition",
  "number": "1.15.14",
  "title": "Units of <span class=\"process-math\">\\(\\mathcal{O}_K\\)<\/span>.",
  "body": " Units of   Let be squarefree, and let . An element is a unit (or invertible ) if there is an element such that . The group of units of is the set of all units: , .   "
},
{
  "id": "prop_quad_ring_units",
  "level": "2",
  "url": "s_ring_integers.html#prop_quad_ring_units",
  "type": "Proposition",
  "number": "1.15.15",
  "title": "Units of <span class=\"process-math\">\\(\\mathcal{O}_K\\)<\/span>.",
  "body": " Units of   Let be squarefree, and let . An element if and only if . In other words .    We prove both implications separately.  If , then there is an element such that . Taking norms, we conclude that . Since and are both integers by , and since the only divisors of are , we conclude that .  Conversely, if , then we see that and thus is an inverse of in . Since if and only if , we see that , showing that .   "
},
{
  "id": "d_norm_euclidean",
  "level": "2",
  "url": "s_ring_integers.html#d_norm_euclidean",
  "type": "Definition",
  "number": "1.15.16",
  "title": "Norm-Euclidean.",
  "body": " Norm-Euclidean   Let be squarefree, and let . The norm  satisfies the division algorithm if given with , there exist satisfying the following conditions.    .     .   We also say that is norm-Euclidean in this case.   "
},
{
  "id": "th_norm_euc",
  "level": "2",
  "url": "s_ring_integers.html#th_norm_euc",
  "type": "Theorem",
  "number": "1.15.17",
  "title": "Norm-Euclidean rings of integers.",
  "body": " Norm-Euclidean rings of integers   Let be squarefree, and let . If is norm-Euclidean then it enjoys the following properties.   Let and assume or . There is a GCD of and , and we have for some .    An element is prime if and only if it is irreducible.    If is a nonzero non-unit, then it has a factorization where is irreducible for all , and this factorization is unique in the sense of .       The proofs of these statements can be adapted immediately from the proofs of the corresponding results for , all of which build on the fact that the norm satisfies the division algorithm.   "
},
{
  "id": "th_quad_ring_irr_fact",
  "level": "2",
  "url": "s_ring_integers.html#th_quad_ring_irr_fact",
  "type": "Theorem",
  "number": "1.15.18",
  "title": "Irreducible factorizations in quadratic rings of integers.",
  "body": " Irreducible factorizations in quadratic rings of integers   Let be a squarefree, let , and let be a nonzero non-unit of . There are irreducible elements , such that .    We will prove by by strong induction on that every element can be factored into irreducibles.  First observe that , and thus that the set is nonempty. Let be the least element of . This is our base case for the induction argument. Assume . If , then , and thus . Since is the minimal absolute norm of elements of , it follows that . But then or is a unit, by . This proves is irreducible.  Now assume the statement is true for all with , and assume . If is irreducible, then we are done. Otherwise, we can write , where and are non-units. It follows that , whence the induction hypothesis applies. We conclude that and have irreducible factorizations. Since , the product of these two irreducible factorizations is an irreducible factorization of .   "
},
{
  "id": "eg_quad_ring_UFD",
  "level": "2",
  "url": "s_ring_integers.html#eg_quad_ring_UFD",
  "type": "Example",
  "number": "1.15.19",
  "title": "<span class=\"process-math\">\\(\\Z[\\sqrt{-5}]\\)<\/span> is not a UFD.",
...
},
{
  ...
  "number": "1.16.7",
  "title": "Unit group of real quadratic extensions.",
  "body": " Unit group of real quadratic extensions   Let be a positive squarefree integer, let , and let and be elements of . We have if and only if .  As a result, the fundamental unit described in is the element of with minimal first coefficient .    Let be the smallest element of , and write , where and for some positive integers . Since , and since for all , it follows that . Since furthermore the function is increasing, we have . Thus for all , we have and for some positive integers and , and if and only if . Let . To finish the proof, it suffices to show that for all , or equivalently, that is an increasing function of . Since , we see that . Recall that since , we have . If , then . The only other option is . In this case we must have . Furthermore, since , we must have , from whence it follows that . But then (since ), and we have . Now we are in the very special case where . Since satisfies , it follows that , and by induction for all . As a result, looking only at the first coefficients of these powers of , we have for all . (And looking at the second coefficients, we see that .) Since for all , we see that , as desired.   "
},
{
  "id": "s_pells_eqn",
  "level": "1",
  ...
  "body": " Pell's equations   Pell's equations   Pell's equations   Let be a positive integer that is not a square. The equation is called Pell's equation . A generalized Pell equation is any equation of the form where is a nonzero integer. The negative Pell equation is the equation .     Dirichlet's approximation theorem   Let be an irrational real number. There are infinitely many integer pairs satisfying , , and .    Fix a positive integer and partition the interval into the disjoint subintervals , where . For each , write . Note that since is irrational, the are themselves irrational (with the exception of ), are distinct, and satisfy . It follows that each lies in exactly one of the intervals , and furthermore, since there are intervals and distinct elements , by the Pigeonhole principle, there are integers , such that for some . But then we have , where , in which case . Moreover, letting , we have . Replacing and with and , we have shown that for any positive integer , we can find a pair of relatively prime integers and with satisfying . To create an infinite sequence of such pairs proceed recursively as follows:   Pick any postive , and let satisfy .    Given pair , pick any positive integer satisfying , and pick satisfying .   For any , we have , from whence it follows that pairs are distinct.     Pell's equation   Let be a positive integer that is not a square. There is a solution to the equation in positive integers.    Let be the set of all pairs satisfying The set is infinite by . The proof will proceed in a number of steps.   We show that there is a nonzero integer such that for infinitely many pairs .    We show that there is an infinite subset of satisfying the following properties:    for all     If , then and .       Associating to pairs the element , we will do some algebra in to construct an element satisfying .      Step 1  For all , we have . Since for all , it follows that assumes finitely many values as ranges over . Since is infinite, there is one value such that for infinitely-many elements of . Since is irrational, it follows that .    Step 2  Let be the set of all satisfying , and consider the map Since is infinite and the set of pairs with is finite, there must be infinitely many pairs that get set sent to the same pair . Let be the set of all such pairs . Any pair in satisfies , and any two pairs in satisfy .    Step 3  Since is infinite, we can find pairs and satisfying and , . Let and consider the elements , of . We have . Since , we see that , and thus we have for some . Now take norms of both sides and simplify: . We conclude that satisfies , and is thus a solution to Pell's equation. It remains only to show that . We have . Since the last equality is excluded by our condition , , we see that .       Negative Pell's equation  Consider now the negative Pell's equation where is a positive squarefree integer. Since is squarefree, we have for distinct primes . If is a solution in positive integers, then for all we have , since as we have seen if and only if or . It follows that a necessary condition for the negative Pell's equation to have a solution is that no prime divisor of is congruent to 3 modulo . Interestingly, this is not in general a sufficient condition . The following recent theorem by Koymans and Pagano , building off of work by Northwestern's own Alex Smith, gives a precise quantitative description of just how far our necessary condition fails to be a sufficient one.   Negative Pell's equation  Koymans, Pagano, 2022   Let be the set of all squarefree positive integers whose prime divisors are congruent to or modulo , and let be the set of squarefree positive integers for which the negative Pell's equation has a solution. We have .    This theorem uses advanced material well outside the scope of this course. However, we can prove the following result, which tells us that in the case where is itself prime, our necessary condition is also sufficient.   Negative Pell equation for prime   Let be a prime integer. The negative Pell equation has a solution if and only if .         "
},
{
  "id": "d_pell_eqn",
  "level": "2",
...
{
  "id": "th_pells_equation",
  "level": "2",
  "url": "s_pells_eqn.html#th_pells_equation",
  "type": "Theorem",
  "number": "1.17.3",
  "title": "Pell’s equation.",
  "body": " Pell's equation   Let be a positive integer that is not a square. There is a solution to the equation in positive integers.    Let be the set of all pairs satisfying The set is infinite by . The proof will proceed in a number of steps.   We show that there is a nonzero integer such that for infinitely many pairs .    We show that there is an infinite subset of satisfying the following properties:    for all     If , then and .       Associating to pairs the element , we will do some algebra in to construct an element satisfying .      Step 1  For all , we have . Since for all , it follows that assumes finitely many values as ranges over . Since is infinite, there is one value such that for infinitely-many elements of . Since is irrational, it follows that .    Step 2  Let be the set of all satisfying , and consider the map Since is infinite and the set of pairs with is finite, there must be infinitely many pairs that get set sent to the same pair . Let be the set of all such pairs . Any pair in satisfies , and any two pairs in satisfy .    Step 3  Since is infinite, we can find pairs and satisfying and , . Let and consider the elements , of . We have . Since , we see that , and thus we have for some . Now take norms of both sides and simplify: . We conclude that satisfies , and is thus a solution to Pell's equation. It remains only to show that . We have . Since the last equality is excluded by our condition , , we see that .    "
},
{
  "id": "th_neg_pells",
  "level": "2",
  "url": "s_pells_eqn.html#th_neg_pells",
  "type": "Theorem",
  "number": "1.17.4",
  "title": "Negative Pell’s equation.",
  "body": " Negative Pell's equation  Koymans, Pagano, 2022   Let be the set of all squarefree positive integers whose prime divisors are congruent to or modulo , and let be the set of squarefree positive integers for which the negative Pell's equation has a solution. We have .   "
},
{
  "id": "th_neg_pell_prime",
  "level": "2",
  "url": "s_pells_eqn.html#th_neg_pell_prime",
  "type": "Theorem",
  "number": "1.17.5",
  "title": "Negative Pell equation for prime <span class=\"process-math\">\\(p\\)<\/span>.",
...
{
  "id": "ss_crypto_intro-5",
  "level": "2",
  "url": "s_crypto.html#ss_crypto_intro-5",
  "type": "Remark",
  "number": "1.18.3",
...
},
{
  ...
  "type": "Table",
  "number": "1.18.10",
  "title": "Frequency of letters, bigrams, and trigrams.",
  "body": " Frequency of letters, bigrams, and trigrams.    Letter Frequency (%) Bigram Frequency (%) Trigram Frequency (%)    e 12.10 th 2.71 the 1.81    t 8.94 he 2.33 and 0.73    a 8.55 in 2.03 ing 0.63    o 7.47 er 1.78 ent 0.42    i 7.33 an 1.61 ion 0.42    n 7.17 re 1.41 her 0.36    "
},
{
  "id": "ss_sub_cipher-7",
  "level": "2",
  "url": "s_crypto.html#ss_sub_cipher-7",
  "type": "Sage example",
  "number": "6",
...
},
{
  ...
  "body": " Vigenère cipher   Let , and let be a positive integer. Define , and encryption methods and as for each key , plaintext and ciphertext .  In this setting a key is called a keyword .   "
},
{
  "id": "sage_vignere",
  "level": "2",
  ...
  "body": " Vigenère cipher: encryption and decryption  Again, using Sage we can easily implement the Vigenère cipher.   Try encrypting and decrpyting a message of your choosing using the Vignère cipher and a keyword of your choosing.   The following is an intercepted ciphertext using the Vignère cipher and the keyword poetry . Find the secret message!    "
},
{
  "id": "d_index_coincidence",
  "level": "2",
  "url": "s_crypto.html#d_index_coincidence",
  "type": "Definition",
  "number": "1.18.12",
...
{
  "id": "ss_vignere_cipher-16",
  "level": "2",
  "url": "s_crypto.html#ss_vignere_cipher-16",
  "type": "Sage example",
  "number": "8",
  "title": "Friedman cryptanalysis of the Vigenère cipher.",
...
},
{
  ...
  "body": " Public key cryptography   Symmetric versus asymmetric cryptosystems  The examples of cryptosystems we have discussed thus far are what is known as symmetric cryptosystems. In these systems, to encrypt and decrypt messages Alice and Bob both use the same key , and thus this shared key must be known to both Alice and Bob somehow. This fact creates a security issue. How do Alice and Bob share the key ? Meeting in person and secretly exchanging a key to use is an option, but of course this has many practical shortcomings, especially when the number of participants in our communication network grows, and keys are frequently changed. Alternatively, they could share the secret key over a communication channel, but now the ever-watchful Eve comes into the picture. It was questions such as these that led Whitfield Diffie and Martin Hellman to propose (in their 1976 paper New directions in Cryptography ) a fundamentally different type of cryptography, known as an asymmetric or public key cryptography.   Public key cryptography (PKC)   A public key (or asymmetric ) cryptosystem is a cryptosystem for which the keyspace consists of pairs , called public\/private key pairs , and such that for all , we have so that encryption is performed using the public key  and decryption is performed using the private key  .  Cryptography that uses public key cryptosystems is called public key cryptography .    Let's leave the formalism of for a moment, and try and understand the asymmetry of this type of system on the level of Alice and Bob.  When Alice uses a public key cryptosystem with encoding\/decoding methods and , she chooses a public\/private key pair , and only makes public the public key component (we say she publishes the public key). Bob can then use the public key and encryption method to encrypt a plaintext message as and send to Alice. Alice can then use her private key to recover as . Note that there are two different manners in which this setup is asymmetric.   The participants in this system use different keys for encryption and decryption.    This system allows for Bob to communicate with Alice, but not conversely, since Bob does not know the key that allows for decryption. In other words, this is a communication with Alice setup, as opposed to a communication between Alice and Bob setup. If Bob also wants to receive messages he can understand, he creates his own public\/private key pair.   More generally to create a communication network of users, so that each user can encrypt outgoing messages and decrypt incoming messages, you have each user create their own public\/private key pair , publish their public key , and keep private their private key . That way, any user can send encrypted messages to user as , and this message can be decrypted by user as . Note that this public key system requires the production of only keys. Using a symmetric cryptosystem for the network, each of the communication channels would require a key, and for security reasons these would all need to be distinct (the more people that know a specific key, the less secure your network). Thus the number of keys grows as for this symmetric design, and only as for the public key design.  How do we actually design a secure public key cryptosystem? Diffie and Hellman articulated the problem in terms of the notion of one-way functions with trapdoor features. Let's go back to the simple Bob-communicating-with-Alice setup. To initiate this communication network, Alice seeks an encryption function that satisfies the following conditions:   Values of the function can be easily computed with sufficient published information.    The information alone of how to compute values of does not allow for In general, it is computationally infeasible to invert , so a ciphertext intercepted by Eve cannot be decrypted as .    Although difficult in general to compute , with some special additional information   proposed the following general approach. We need to find a mathematical function that is easy to compute, but hard to invert. Such a function is called a one-way function . If we can find such a function, then we can use it to create a public key cryptosystem as follows. Alice chooses a one-way function , and publishes the description of this function as her public key. To encrypt a message , Bob computes the ciphertext . To decrypt the message, Alice uses her private key, which is the trapdoor information that allows her to efficiently compute an inverse of , to recover from . Thus, the security of this system relies on the fact that without the trapdoor information, it is computationally infeasible for Eve to invert the one-way function and recover the plaintext message from the ciphertext.    Diffie-Hellman key exchange  Diffie and Hellman did not themselves propose an actual example of a public key cryptosystem, but they did describe the following key exchange that makes use of their one-way and trapdoor function concepts.   Diffie-Hellman key exchange   Alice and Bob wish to share a key over an insecure communication channel. The Diffie-Hellman key exchange accomplishes this as follows.   Public  Alice and Bob agree publicly on a prime and a primitive root of .    Private  Alice chooses a random integer exponent , and Bob chooses a random integer exponent . These exponents are kept private.    Exchange  Alice computes and sends this value to Bob; Bob computes and sends this value to Alice.    Shared key  Alice uses her private exponent to compute ; and Bob uses his private exponent to compute . Since , Alice and Bob are now in possession of a shared key .        Integers or congruence classes  We have chosen to use the language of to express the mathematics behind the Diffie-Hellman key exchange; and we will do the same with the Elgamal and RSA cryptosystems below. As usual, we could just as well have described things solely in terms of integers, in which case we would have to make use of the least positive residue function . For example, here is an integer-based summary of the Diffie-Hellman key exchange.   Pick a prime and an integer that is a primitive root modulo .    Alice chooses a random integer exponent , computes , and sends it to Bob; Bob chooses a random integer exponent , computes , and sends it to Alice.    Alice and Bob use their private exponents and to compute the shared key .   As you can see, our notational choice has the advantage of notational brevity. The integer-based description is more common in the literature, however, and implemented versions of these systems use integers, not congruence classes.    Fast modular exponentiation  The Diffie-Hellman key exchange, as well as the Elgamal and RSA encryption methods below all rely on modular exponentiation. For these various methods to be efficiently used, this operation needs to be performed reasonably quickly. This is possible thanks to a fast method of modular exponentiation that is based on the binary expansion of the exponent. See the Rosen text for more details (or consult your notes from Math 336-1).   Before getting to our computational example , we first point out an obvious potential weakness to the Diffie-Hellman key exchange. Namely, if Eve can intercept the transmissions and , then since is publicly known, she could simply find the exponent and then compute . In number theory terms, Eve here has computed the discrete log (or index ) of to the base . Whether or not the Diffie-Hellman key exchange is vulnerable to this particular attack depends on how easily this discrete log computation can be performed: , it depends on the difficulty of the discrete log problem .   Discrete log (or index)   Let be a positive integer, let be a unit of order , and let . The unique integer such that is called the discrete log (or index ) of to the base , and is denoted by (or ).    Finding discrete logs is important enough to number theory and cryptography that it has earned its own name as a computational problem: the discrete log problem .   Discrete log problem   The discrete log problem for modular unit groups is the general problem described as follows: given a positive integer , unit , and element , find the unique integer such that ( , compute ).    Eve's discrete log attack on the Diffie-Hellman key exchange is just the most obvious one. There may be other attacks, and indeed, cracking a general Diffie-Hellman key exchange may be easier than solving the discrete log problem. Accordingly, we give this computational problem its own name.   Diffie-Hellman problem   The Diffie-Hellman problem is the general problem described as follows: given prime integer , primitive root , and powers and , compute the power .     Discrete log and Diffie-Hellman problems     It is clear that a solution to the discrete log problem could be used to solve the Diffie-Hellman problem. The converse is unknown: that is, it is not known whether a solution to the Diffie-Hellman problem would give rise to a solution to the discrete log problem.    The discrete log problem for general prime is considered to be difficult to solve in general. In particular there is no known algorithm for computing discrete logs modulo whose runtime is polynomial in .    The discrete log problem can be solved efficiently if has small prime factors, thanks to the Pohlig-Hellman algorithm (an application of the CRT). Since is always even (for large primes), a Diffie-Hellman key exchange protocol should us a prime such that , where is a large prime, or equivalently, . Such primes are called safe primes in the cryptographic literature.       Diffie-Hellman implementation  Let's implement the Diffie-Hellman key exchange protocol, taking some care to choose a large safe prime. We first create a list of primes of the form of a largish size, where is prime.   Next let's perform a key exchange with one of these primes. We will do our arithmetic within , as opposed to using congruences.   Now Alice and Bob will choose their random private exponents, exchange their computed powers, and compute the shared key.   Could Eve have determined by solving the discrete log problem in this case? Let's see if Sage can do it. We try to use Sage's built-in discrete log function log() .   It appears that primes in the range do not give rise to secure Diffie-Hellman key exchanges. See what happens when you replace qrange=2^90 with qrange=2^100 , qrange=2^110 , . You will see that the Sage discrete log function log() running on a Sage finite field GF(p) quickly reaches its limitations.     Encoding to integers  Below we discuss two public key cryptosystems, Elgamal and RSA. Both systems use as their plaintext and ciphertext spaces a modular ring for some integer (or if you prefer to work with integers). In other words the plaintexts that are encrypted and decrypted are themselves just congruence classes . This might seem somewhat limiting. What's the use of communicating integer messages to one another? Isn't a string of characters a more natural thing to communicate? Indeed, it is, but we can easily encode strings of characters as integers.  For example, the ASCII system assigns to each integer a character. The Sage commands chr() and ord() convert integers in this range to characters, and inversely. The first 32 integers of the ASCII range are reserved for non-printing control characters with string literals like \\n (newline) and \\t (tab).    The ASCII functions give us a way to encode individual characters as integers lying in . To encode an entire string of characters as an integer, we can convert each character to its ASCII integer , and then assign to the string the integer with base-256 expansion : , we set .     Elgamal encryption  The Elgamal public key cryptosystem was first described in a 1985 paper by Taher Elgamal titled A public key cryptosystem and a signature scheme based on discrete logarithms . Although chronilogically more recent than the RSA cryptosystem, we treat it first for its obvious connection to the Diffie-Hellman key exchange.   Elgamal encryption   Bob wishes to send messages to Alice over an insecure channel. The Elgamal cryptosystem accomplishes this as follows.   Public  Alice publishes a large prime and a primitive root of .    Private  Alice chooses a random integer exponent , and keeps this exponent private.    Public key  Alice computes and publishes this value as her public key.    Encryption  To encrypt a message , Bob chooses a random integer , computes the elements and in , and sends the ciphertext to Alice.    Decryption  To decrypt the message, Alice uses her private key to compute .        Proof of correctness of Elgamal  With notation as in , it is easy to see that . Indeed, we have .    Elgamal encryption and decryption   Below you find the relevant Sage functions for Elgamal encryption and decryption. We'e included our str2int() and int2str functions discussed in . Coding-wise, it is slighly more convenient here to work with integers, as opposed to congruence classes. You will notice that we use power_mod(g,k,p) which is a fast modular exponentiation function to compute . Encoding-wise, we are using a smaller character set so that our messages encode to integers that are smaller than our prime .   Now we will have Alice set up a public key . As with the Diffie-Hellman key exchange, Alice will choose a large safe prime , with prime. We are going for a prime in the range here. You will notice that Sage has to do some work here, but it will eventually return a prime.   Now Alice creates the rest of her public key, keeping her integer exponent private.   Bob now encodes a message as an integer and encrypts it using the Elgamal public key .   And now Alice decrypts the message using her private key , making sure to convert the resulting integer back to a string.       RSA Encryption  The RSA public key cryptosystem predates Elgamal. Its name is derived from the last names of its co-creators Ron Rivest, Adi Shamir, and Leonard Adleman, who published a description of the cryptosystem in their 1977 paper A method for obtaining digital signatures and public-key cryptosystems . Our discussion of public key cryptosystems began with the later system Elgamal (1985) mainly because of its close connection to the Diffie-Hellman key exchange and the discrete log problem. In contrast, the security of RSA, as we will soon see, is connected with the problem of factoring large integers.   RSA encryption   Bob wishes to send messages to Alice over an insecure channel. The RSA cryptosystem accomplishes this as follows.   Private  Alice chooses two distinct primes and that she keeps private.    Public key  Alice publishes her RSA modulus  and an integer such that . The public key is thus the pair .    Private key  Alice computes an integer such that . She is able to do this efficiently, since she knows , can thus easily compute , and then use the extended Euclidean algorithm to find a multiplicative inverse of modulo .  The integer is Alice's private key.    Encryption  To encrypt a message , Bob computes the ciphertext and sends this to Alice.    Decryption  To decrypt the message, Alice uses her private key to compute .        Proof of correctness of RSA  With notation as in , we will show that given and , we have . We are tempted to apply Euler's theorem ( for all as follows: write for some integer , and then compute . The only issue here is that the plaintext is not necessarily an element of . Since , however, the only exceptions we need to deal with are , , and . It is clear that when . For the other two cases, we use the CRT and Fermat's little theorem. Assume that . We have , where we have used Fermat's little theorem in the second congruence to conclude . Since the integers and both solve the system of congruences , we conclude by the uniqueness claim of the CRT that and hence that . The case where is exactly similar.    RSA encryption and decryption  Let's look at all of the ingredients of RSA to make sure all the required computations can be done efficiently.   Since encryption and decryption are both instances of modular exponentiation, both can be done reasonably quickly even for large , , and using fast modular exponentiation.    Since Alice knows and , she can easily compute . Once this is known, she can use the Euclidean algorithm (which is reasonably fast) to find an integer with and its multiplicative inverse modulo .       Computational problems related to RSA  The most obvious threat to RSA is the ability to factor . If Eve can find the factorization , then just like Alice, she can easily compute and the private key . As with our discussion of the security of the Diffie-Hellman key exchange, it is useul to identify a number of computational problems related to RSA.   RSA problem  Given RSA modulus and exponent , and ciphertext , compute .    Factoring problem  Given positive integer , find the prime factorization of .    -factoring problem  Given known to be a product of two distinct primes, factor .    problem  Given integer known to be a product of two distinct primes, compute .   As discussed above, a solution to the -factoring problem gives rise to a solution to the RSA problem. It is not known whether the converse is true: , we do not know if breaking RSA is equivalent to factoring products of two distinct primes.  It is also clear that a general solution to the -factoring problem yields a general solution to the problem. Interestingly, the converse is also true: that is, a general solution to the problem gives rise to a general solution of the -factoring problem. (You will show this in your homework.) As a result, it is just as important for Alice to keep a secret, as it is for her to keep and secret.  It is generally believed that there is no efficient algorithm for factoring large integers that can be implemented on a classical computer, and thus that RSA is secure against this most obvious of attacks. There is however an efficient method for factoring large integers, Shor's algorithm , that can be implemented on a quantum computer. This means that if a sufficiently powerful quantum computer can be built, then RSA will no longer be secure. This is one the main motivations behind research into so-called post-quantum cryptosystems.   Let's begin with a small RSA example, just to remind ourselves of some of the number theoretic algorithms at work.   RSA: toy example   Encode, and then decode the integer message using the RSA public key .    Note that we have expressed things in terms of integers, as opposed to congruence classes. To encode, we must compute . This will give us a chance to illustrate the idea behind fast modular exponentiation. Writing we see that we can compute as . Thus the encoded ciphertext is . Note that, knowing that , we could have made our life even easier by computing and , and then using the CRT to conclude . However, in the RSA setting this technique should not be available to an encryprter, since they should not know the factorization of .  To decode, we must find a multiplicateive inverse of modulo . The most direct way to do this is by applying the extended Euclidean algorithm to the pair . Since , we see that , and thus . Thus we can choose any in the congruence class as our private key. To decode the ciphertext , we must compute . Sage can do this computation for us with no difficulties.   To compute this by hand, we could use fast modular exponentiation again, writing . Alternatively, we can compute , and conclude by the CRT that . Thus , as expected.     RSA: encryption and decryption   We include below some encoding\/decoding functions and RSA encryption\/decrpytion functions.   Below we are setting up an RSA public key that uses as the public exponent. For that reason, we make sure to pick our random large primes and so that .   In the Sage cell below, choose your favorite lowercase string s , encode it as an integer, encrypt and decrypt it using the public key , and decode back to a string.   Below you find an intercepted RSA ciphertext , the result of encoding a message as an integer using str2int(s) and then encrypting this integer using the RSA public key . Using the provided informtation of and (named phiN ), find the secret message! (Hint: the command power_mod(a,b,M) computes for any integer power , even negative ones!)       "
},
{
  "id": "d_public_key",
  "level": "2",
...
},
{
  "id": "ss_public_key-6",
  "level": "2",
  "url": "s_public_key.html#ss_public_key-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "one-way function "
},
{
  "id": "d_diffie_hellman",
  "level": "2",
  ...
  "body": " Diffie-Hellman key exchange   Alice and Bob wish to share a key over an insecure communication channel. The Diffie-Hellman key exchange accomplishes this as follows.   Public  Alice and Bob agree publicly on a prime and a primitive root of .    Private  Alice chooses a random integer exponent , and Bob chooses a random integer exponent . These exponents are kept private.    Exchange  Alice computes and sends this value to Bob; Bob computes and sends this value to Alice.    Shared key  Alice uses her private exponent to compute ; and Bob uses his private exponent to compute . Since , Alice and Bob are now in possession of a shared key .      "
},
{
  "id": "ss_diffie_hellman-4",
  "level": "2",
  "url": "s_public_key.html#ss_diffie_hellman-4",
  "type": "Remark",
  "number": "1.19.3",
  "title": "Integers or congruence classes.",
  "body": " Integers or congruence classes  We have chosen to use the language of to express the mathematics behind the Diffie-Hellman key exchange; and we will do the same with the Elgamal and RSA cryptosystems below. As usual, we could just as well have described things solely in terms of integers, in which case we would have to make use of the least positive residue function . For example, here is an integer-based summary of the Diffie-Hellman key exchange.   Pick a prime and an integer that is a primitive root modulo .    Alice chooses a random integer exponent , computes , and sends it to Bob; Bob chooses a random integer exponent , computes , and sends it to Alice.    Alice and Bob use their private exponents and to compute the shared key .   As you can see, our notational choice has the advantage of notational brevity. The integer-based description is more common in the literature, however, and implemented versions of these systems use integers, not congruence classes.  "
},
{
  "id": "ss_diffie_hellman-5",
  "level": "2",
  "url": "s_public_key.html#ss_diffie_hellman-5",
  "type": "Remark",
  "number": "1.19.4",
  "title": "Fast modular exponentiation.",
  "body": " Fast modular exponentiation  The Diffie-Hellman key exchange, as well as the Elgamal and RSA encryption methods below all rely on modular exponentiation. For these various methods to be efficiently used, this operation needs to be performed reasonably quickly. This is possible thanks to a fast method of modular exponentiation that is based on the binary expansion of the exponent. See the Rosen text for more details (or consult your notes from Math 336-1).  "
},
{
  "id": "d_discrete_log",
  "level": "2",
  ...
  "body": " Discrete log (or index)   Let be a positive integer, let be a unit of order , and let . The unique integer such that is called the discrete log (or index ) of to the base , and is denoted by (or ).   "
},
{
  ...
  "body": " Discrete log problem   The discrete log problem for modular unit groups is the general problem described as follows: given a positive integer , unit , and element , find the unique integer such that ( , compute ).   "
},
{
  "id": "d_diffie-hellman_problem",
  "level": "2",
  "url": "s_public_key.html#d_diffie-hellman_problem",
  "type": "Definition",
  "number": "1.19.7",
  "title": "Diffie-Hellman problem.",
  "body": " Diffie-Hellman problem   The Diffie-Hellman problem is the general problem described as follows: given prime integer , primitive root , and powers and , compute the power .   "
},
{
  "id": "ss_diffie_hellman-12",
  "level": "2",
  "url": "s_public_key.html#ss_diffie_hellman-12",
  "type": "Remark",
  "number": "1.19.8",
  "title": "Discrete log and Diffie-Hellman problems.",
  "body": " Discrete log and Diffie-Hellman problems     It is clear that a solution to the discrete log problem could be used to solve the Diffie-Hellman problem. The converse is unknown: that is, it is not known whether a solution to the Diffie-Hellman problem would give rise to a solution to the discrete log problem.    The discrete log problem for general prime is considered to be difficult to solve in general. In particular there is no known algorithm for computing discrete logs modulo whose runtime is polynomial in .    The discrete log problem can be solved efficiently if has small prime factors, thanks to the Pohlig-Hellman algorithm (an application of the CRT). Since is always even (for large primes), a Diffie-Hellman key exchange protocol should us a prime such that , where is a large prime, or equivalently, . Such primes are called safe primes in the cryptographic literature.     "
},
{
  "id": "sage_diffie_hellman",
  "level": "2",
  "url": "s_public_key.html#sage_diffie_hellman",
  "type": "Sage example",
  "number": "9",
  "title": "Diffie-Hellman implementation.",
  "body": " Diffie-Hellman implementation  Let's implement the Diffie-Hellman key exchange protocol, taking some care to choose a large safe prime. We first create a list of primes of the form of a largish size, where is prime.   Next let's perform a key exchange with one of these primes. We will do our arithmetic within , as opposed to using congruences.   Now Alice and Bob will choose their random private exponents, exchange their computed powers, and compute the shared key.   Could Eve have determined by solving the discrete log problem in this case? Let's see if Sage can do it. We try to use Sage's built-in discrete log function log() .   It appears that primes in the range do not give rise to secure Diffie-Hellman key exchanges. See what happens when you replace qrange=2^90 with qrange=2^100 , qrange=2^110 , . You will see that the Sage discrete log function log() running on a Sage finite field GF(p) quickly reaches its limitations.  "
},
{
  "id": "d_elgamal",
  "level": "2",
  "url": "s_public_key.html#d_elgamal",
  "type": "Definition",
  "number": "1.19.9",
  "title": "Elgamal encryption.",
  "body": " Elgamal encryption   Bob wishes to send messages to Alice over an insecure channel. The Elgamal cryptosystem accomplishes this as follows.   Public  Alice publishes a large prime and a primitive root of .    Private  Alice chooses a random integer exponent , and keeps this exponent private.    Public key  Alice computes and publishes this value as her public key.    Encryption  To encrypt a message , Bob chooses a random integer , computes the elements and in , and sends the ciphertext to Alice.    Decryption  To decrypt the message, Alice uses her private key to compute .      "
},
{
  "id": "ss_elgamal-4",
  "level": "2",
  "url": "s_public_key.html#ss_elgamal-4",
  "type": "Proof",
  "number": "1.19.4.1",
  "title": "Proof of correctness of Elgamal.",
  "body": " Proof of correctness of Elgamal  With notation as in , it is easy to see that . Indeed, we have .  "
},
{
  "id": "sage_elgamal",
  "level": "2",
  "url": "s_public_key.html#sage_elgamal",
  "type": "Sage example",
  "number": "10",
  "title": "Elgamal encryption and decryption.",
  "body": " Elgamal encryption and decryption   Below you find the relevant Sage functions for Elgamal encryption and decryption. We'e included our str2int() and int2str functions discussed in . Coding-wise, it is slighly more convenient here to work with integers, as opposed to congruence classes. You will notice that we use power_mod(g,k,p) which is a fast modular exponentiation function to compute . Encoding-wise, we are using a smaller character set so that our messages encode to integers that are smaller than our prime .   Now we will have Alice set up a public key . As with the Diffie-Hellman key exchange, Alice will choose a large safe prime , with prime. We are going for a prime in the range here. You will notice that Sage has to do some work here, but it will eventually return a prime.   Now Alice creates the rest of her public key, keeping her integer exponent private.   Bob now encodes a message as an integer and encrypts it using the Elgamal public key .   And now Alice decrypts the message using her private key , making sure to convert the resulting integer back to a string.    "
},
{
  "id": "d_rsa",
  "level": "2",
  "url": "s_public_key.html#d_rsa",
  "type": "Definition",
  "number": "1.19.10",
  "title": "RSA encryption.",
  "body": " RSA encryption   Bob wishes to send messages to Alice over an insecure channel. The RSA cryptosystem accomplishes this as follows.   Private  Alice chooses two distinct primes and that she keeps private.    Public key  Alice publishes her RSA modulus  and an integer such that . The public key is thus the pair .    Private key  Alice computes an integer such that . She is able to do this efficiently, since she knows , can thus easily compute , and then use the extended Euclidean algorithm to find a multiplicative inverse of modulo .  The integer is Alice's private key.    Encryption  To encrypt a message , Bob computes the ciphertext and sends this to Alice.    Decryption  To decrypt the message, Alice uses her private key to compute .      "
},
{
  "id": "ss_rsa_enc-4",
  "level": "2",
  "url": "s_public_key.html#ss_rsa_enc-4",
  "type": "Proof",
  "number": "1.19.5.1",
  "title": "Proof of correctness of RSA.",
  "body": " Proof of correctness of RSA  With notation as in , we will show that given and , we have . We are tempted to apply Euler's theorem ( for all as follows: write for some integer , and then compute . The only issue here is that the plaintext is not necessarily an element of . Since , however, the only exceptions we need to deal with are , , and . It is clear that when . For the other two cases, we use the CRT and Fermat's little theorem. Assume that . We have , where we have used Fermat's little theorem in the second congruence to conclude . Since the integers and both solve the system of congruences , we conclude by the uniqueness claim of the CRT that and hence that . The case where is exactly similar.  "
},
{
  "id": "ss_rsa_enc-5",
  "level": "2",
  "url": "s_public_key.html#ss_rsa_enc-5",
  "type": "Remark",
  "number": "1.19.11",
  "title": "RSA encryption and decryption.",
  "body": " RSA encryption and decryption  Let's look at all of the ingredients of RSA to make sure all the required computations can be done efficiently.   Since encryption and decryption are both instances of modular exponentiation, both can be done reasonably quickly even for large , , and using fast modular exponentiation.    Since Alice knows and , she can easily compute . Once this is known, she can use the Euclidean algorithm (which is reasonably fast) to find an integer with and its multiplicative inverse modulo .     "
},
{
  "id": "ss_rsa_enc-6",
  "level": "2",
  "url": "s_public_key.html#ss_rsa_enc-6",
  "type": "Remark",
  "number": "1.19.12",
  "title": "Computational problems related to RSA.",
  "body": " Computational problems related to RSA  The most obvious threat to RSA is the ability to factor . If Eve can find the factorization , then just like Alice, she can easily compute and the private key . As with our discussion of the security of the Diffie-Hellman key exchange, it is useul to identify a number of computational problems related to RSA.   RSA problem  Given RSA modulus and exponent , and ciphertext , compute .    Factoring problem  Given positive integer , find the prime factorization of .    -factoring problem  Given known to be a product of two distinct primes, factor .    problem  Given integer known to be a product of two distinct primes, compute .   As discussed above, a solution to the -factoring problem gives rise to a solution to the RSA problem. It is not known whether the converse is true: , we do not know if breaking RSA is equivalent to factoring products of two distinct primes.  It is also clear that a general solution to the -factoring problem yields a general solution to the problem. Interestingly, the converse is also true: that is, a general solution to the problem gives rise to a general solution of the -factoring problem. (You will show this in your homework.) As a result, it is just as important for Alice to keep a secret, as it is for her to keep and secret.  It is generally believed that there is no efficient algorithm for factoring large integers that can be implemented on a classical computer, and thus that RSA is secure against this most obvious of attacks. There is however an efficient method for factoring large integers, Shor's algorithm , that can be implemented on a quantum computer. This means that if a sufficiently powerful quantum computer can be built, then RSA will no longer be secure. This is one the main motivations behind research into so-called post-quantum cryptosystems.  "
},
{
  "id": "eg_RSA_toy",
  "level": "2",
  "url": "s_public_key.html#eg_RSA_toy",
  "type": "Example",
  "number": "1.19.13",
  "title": "RSA: toy example.",
  "body": " RSA: toy example   Encode, and then decode the integer message using the RSA public key .    Note that we have expressed things in terms of integers, as opposed to congruence classes. To encode, we must compute . This will give us a chance to illustrate the idea behind fast modular exponentiation. Writing we see that we can compute as . Thus the encoded ciphertext is . Note that, knowing that , we could have made our life even easier by computing and , and then using the CRT to conclude . However, in the RSA setting this technique should not be available to an encryprter, since they should not know the factorization of .  To decode, we must find a multiplicateive inverse of modulo . The most direct way to do this is by applying the extended Euclidean algorithm to the pair . Since , we see that , and thus . Thus we can choose any in the congruence class as our private key. To decode the ciphertext , we must compute . Sage can do this computation for us with no difficulties.   To compute this by hand, we could use fast modular exponentiation again, writing . Alternatively, we can compute , and conclude by the CRT that . Thus , as expected.   "
},
{
  "id": "sage_rsa",
  "level": "2",
  "url": "s_public_key.html#sage_rsa",
  "type": "Sage example",
  "number": "11",
  "title": "RSA: encryption and decryption.",
  "body": " RSA: encryption and decryption   We include below some encoding\/decoding functions and RSA encryption\/decrpytion functions.   Below we are setting up an RSA public key that uses as the public exponent. For that reason, we make sure to pick our random large primes and so that .   In the Sage cell below, choose your favorite lowercase string s , encode it as an integer, encrypt and decrypt it using the public key , and decode back to a string.   Below you find an intercepted RSA ciphertext , the result of encoding a message as an integer using str2int(s) and then encrypting this integer using the RSA public key . Using the provided informtation of and (named phiN ), find the secret message! (Hint: the command power_mod(a,b,M) computes for any integer power , even negative ones!)     "
},
{
